        ...
        for i in range(self.config.max_tries):
            ...
            if self.success_checker(result):
                ...
                if not last_iteration:
                    ...
                    logging.info("Sleeping %d seconds until retrying again.",
                                 next_sleep)
                    self.sleeper(next_sleep if self.config.max_sleep <= 0 else
                                 min(next_sleep, self.config.max_sleep))
                    next_sleep *= self.config.sleep_factor
                else:
                    logging.warning(
                        "Giving up execution. Failed too many times.")
...
class TokenContainer:
    ...
    def get_local_nodes(self):
        ...
        if not self.options.datacenter:
            logging.debug(
                "No datacenter specified, all ring members' tokens will be considered"
            )
            return
        ...
        # revisited.
        search_regex = "DC(?::\d+)?:{datacenter}".format(
            datacenter=self.options.datacenter)
    ...
    def get_ring_tokens(self):
        ...
        """
        logging.info(
            "running nodetool ring, this will take a little bit of time")
        ...
        for line in stdout.split("\n")[4:]:
            ...
            if self.options.datacenter and not segments[0] in self.local_nodes:
                logging.debug("Discarding node/token %s/%s", segments[0],
                              segments[-1])
                continue
    ...
    def get_host_tokens(self):
        ...
        if not success or stdout.find("Token") == -1:
            ...
            raise Exception(
                "Died in get_host_tokens, success: %d, stderr: %s" %
                (success, stderr))
    ...
    def sub_range_generator(self, start, stop, steps=100):
        ...
        if stop > start:
            if start + steps < stop + 1:
                ...
                step_list = [
                    self.format(x) for x in range(start, stop, step_increment)
                ][0:steps]
            else:
                step += 1
            ...
            if distance > steps - 1:
                ...
                step_list = [
                    self.format(x)
                    for x in range(start, self.RANGE_MAX, step_increment)
                ]
                step_list.extend([
                    self.format(x)
                    for x in range(self.RANGE_MIN, stop, step_increment)
                ])
...
class RepairStatus(object):
    ...
    def repair_start(self,
        ...
        """
        self.current_repair = self._build_repair_dict(cmd, step, start, end,
                                                      nodeposition, keyspace,
                                                      column_families)
        self.write()

    def repair_fail(self,
                    cmd,
                    step,
                    start,
                    end,
        ...
        self.failed_repairs.append(
            self._build_repair_dict(cmd, step, start, end, nodeposition,
                                    keyspace, column_families))
        self.failed_count += 1
        self.write()
...
def repair_range(options, start, end, step, nodeposition, repair_status=None):
    ...
    if options.exclude_step or options.offset:
        ...
        if excluded == 1:
            logging.debug(
                "{nodeposition} step {step:04d} skipping range ({start}, {end}) for keyspace {keyspace}"
                .format(step=step,
                        start=start,
                        end=end,
                        nodeposition=nodeposition,
            ...
            logging.info(
                'Running individual repair commands for each keyspace to exclude {0} {1}'
                .format(options.exclude_step['keyspace'],
                        options.exclude_step['column_family'] or ''))
            for keyspace, column_families in enumerate_keyspaces(
                    options).iteritems():
                if keyspace == options.exclude_step['keyspace']:
                    if options.exclude_step['column_family']:
                        logging.info(
                            'Repairing all column families except {0} for keyspace {1}'
                            .format(options.exclude_step['column_family'],
                                    keyspace))
                        cf_to_repair = [
                            cf for cf in column_families
                            if cf != options.exclude_step['column_family']
                        ]
                        _repair_range(options, start, end, step, nodeposition,
                                      keyspace, cf_to_repair, repair_status)
                        continue
                    else:
                        logging.debug(
                            "{nodeposition} step {step:04d} skipping range ({start}, {end}) for keyspace {keyspace}"
                        ...
                        continue
                _repair_range(options, start, end, step, nodeposition,
                              keyspace, options.columnfamily, repair_status)
            return
    # Normal repair_range
    _repair_range(options, start, end, step, nodeposition, options.keyspace,
                  options.columnfamily, repair_status)


def _repair_range(options,
                  start,
                  end,
                  step,
                  nodeposition,
    ...
    """
    logging.debug(
        "{nodeposition} step {step:04d} repairing range ({start}, {end}) for keyspace {keyspace}"
        .format(step=step,
                start=start,
                end=end,
                nodeposition=nodeposition,
    ...
    cmd.extend([options.snapshot, "-st", start, "-et", end])

    if repair_status:
        repair_status.repair_start(cmd, step, start, end, nodeposition,
                                   keyspace, column_families)

    if not options.dry_run:
        retry_options = ExponentialBackoffRetryerConfig(
            options.max_tries, options.initial_sleep, options.sleep_factor,
            options.max_sleep)
        retryer = ExponentialBackoffRetryer(retry_options, lambda x: x[0],
                                            run_command)
        success, cmd, _, stderr = retryer(*cmd)
    else:
        if repair_status:
            repair_status.repair_success(cmd, step, start, end, nodeposition,
                                         keyspace, column_families)
        print(
            "{step:04d}/{nodeposition}".format(nodeposition=nodeposition,
                                               step=step),
            " ".join([str(x) for x in cmd]))
        success = True
    if not success:
        if repair_status:
            repair_status.repair_fail(cmd, step, start, end, nodeposition,
                                      keyspace, column_families)
        logging.error("FAILED: {nodeposition} step {step:04d} {cmd}".format(
            nodeposition=nodeposition, step=step, cmd=cmd))
        logging.error(stderr)
        return
    logging.debug("{nodeposition} step {step:04d} complete".format(
        nodeposition=nodeposition, step=step))
    return
    ...
    try:
        for token_num, host_token in enumerate(tokens.host_tokens):
            ...
            if options.offset and token_num < options.offset['node'] - 1:
                logging.info("[{count}/{total}] skipping token..".format(
                    count=token_num + 1, total=tokens.host_token_count))
                continue
            logging.info(
                "[{count}/{total}] repairing range ({token}, {termination}) in {steps} steps for keyspace {keyspace}"
                .format(count=token_num + 1,
            ...
            results = [
                worker_pool.apply_async(
                    repair_range,
                    (options, start, end, step, "{count}/{total}".format(
                        count=token_num + 1,
                        total=tokens.host_token_count), repair_status))
                for start, end, step in tokens.sub_range_generator(
                    range_start, range_termination, options.steps)
            ]
            for r in results:
                r.get()
...
def is_excluded(options, start, end, step, nodeposition):
    ...
    if options.offset and options.offset['node'] == int(
            current_node) and step < options.offset['step']:
        return 1

    if options.exclude_step and options.exclude_step[
            'node'] == current_node and options.exclude_step['step'] == step:
        if options.exclude_step['keyspace']:
            if options.keyspace and options.keyspace == options.exclude_step[
                    'keyspace']:
                return 1
...
def parse_offset(option, opt_str, value, parser):
    ...
    pieces = value.split(',')
    if len(pieces) >= 2:
        offset = {'node': int(pieces[0]), 'step': int(pieces[1])}
    else:
        offset = {'node': int(pieces[0]), 'step': 1}
...
def main():
    ...
    parser.add_option("-H",
                      ...
                      metavar="HOST",
                      help="Hostname to repair [default: %default]")
    ...
    parser.add_option(
        ...
        type="int",
        metavar="PORT",
        help="JMX port to use for nodetool commands [default: %default]")
    ...
    parser.add_option("-u",
                      ...
                      default=None,
                      metavar="USERNAME",
                      help="Username to use for nodetool commands")
    ...
    parser.add_option("-p",
                      ...
                      default=None,
                      metavar="PASSWORD",
                      help="Password to use for nodetool commands")
    ...
    parser.add_option("-s",
                      ...
                      default=100,
                      metavar="STEPS",
                      help="Number of discrete ranges [default: %default]")
    ...
    parser.add_option("-o",
                      ...
                      type="str",
                      metavar="OFFSET",
                      help="Start from node[,step]",
                      callback=parse_offset)
    ...
    parser.add_option("-n",
                      ...
                      default="nodetool",
                      metavar="NODETOOL",
                      help="Path to nodetool [default: %default]")
    ...
    parser.add_option(
        ...
        default=1,
        metavar="WORKERS",
        help="Number of workers to use for parallelism [default: %default]")
    ...
    parser.add_option("-D",
                      ...
                      default=None,
                      metavar="DATACENTER",
                      help="Identify local datacenter [default: %default]")
    ...
    parser.add_option("-l",
                      ...
                      const="-local",
                      metavar="LOCAL",
                      help="Restrict repair to the local DC")
    ...
    parser.add_option("-i",
                      ...
                      action='store_true',
                      metavar="INC",
                      help="Carry out an incremental repair (post-2.1 only).")
    ...
    parser.add_option("-S",
                      ...
                      const="-snapshot",
                      metavar="LOCAL",
                      help="Use snapshots (pre-2.x only)")
    ...
    parser.add_option(
        ...
        action="callback",
        type="str",
        help="Exclude a [keyspace,[column_family,]]node,step in repairs",
        callback=parse_exclude_step)

    parser.add_option("--output-status",
                      dest="output_status",
                      help="Output (and update) a status file for each run")

    expBackoffGroup = OptionGroup(
        parser, "Exponential backoff options",
    ...
    expBackoffGroup.add_option(
        ...
        default=1,
        help=
        "Number of times to rerun a failed `nodetool repair` call [default: %default]"
    )
    ...
    expBackoffGroup.add_option(
        ...
        default=1,
        help=
        "Number of seconds to sleep first `nodetool repair` [default: %default]"
    )
    ...
    expBackoffGroup.add_option(
        ...
        default=2,
        help=
        ("Multiplication factor that sleep time increases with for every failed"
    ...
    expBackoffGroup.add_option(
        ...
        default=1800,
        help=
        ("Maximum time in seconds the retryer is allowed to sleep. Set to zero or"
